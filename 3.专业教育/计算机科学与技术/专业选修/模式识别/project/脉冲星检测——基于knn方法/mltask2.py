...
posdata=0
negdata=0
for i in range(lendata):  # find out the negative data and positive data of data
	if data[i][8]==0:
		negdata=negdata+1
	if data[i][8]==1:
		posdata=posdata+1
...
N=6
train=data[0:postrain_num]+data[posdata:posdata+postrain_num*N]  # extract train data from data
# extract test data from data
test=data[postrain_num:posdata]+data[posdata+int(0.9*negdata):posdata+int(0.9*negdata)+(posdata-postrain_num)*N]
